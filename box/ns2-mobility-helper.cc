      ...
      while (!file.eof () )
        {
          ...
          if (IsSetInitialPos (pr))
          ...
          else // NOW EVENTS TO BE SCHEDULED
            {
               ...
               * line like $ns_ at 1 "$node_(0) setdest 2 3 4 4.2"
               */
              if (IsSchedMobilityPos (pr))
                {
                  ...
                  //                                     last position     time  X coord     Y coord   z coord   velocity
                  last_pos[iNodeId] = SetMovement (model, last_pos[iNodeId].m_finalPosition, at, pr.dvals[5], pr.dvals[6], pr.dvals[7], pr.dvals[8]);
...
ParseNs2Line (const std::string& str)
{
  ...
  if ( (tokensLength == 7 || tokensLength == 8 || tokensLength == 9)
    ...
    {
      // if the line has the " character in this way: $ns_ at 1 "$node_(0) setdest 2 2 1 3 "
...
GetNodeIdInt (ParseResult pr)
{
  ...
  switch (pr.tokens.size ())
    ...
    case 8:   // line like $ns_ at 1 "$node_(0) setdest 2 3 4"
      result = pr.ivals[3];
      break;
    case 9:  // line like $ns_ at 1 "$node_(0) setdest 2 3 4 5"
      result = pr.ivals[3];
      break;
    default:
      result = -1;
...
IsSchedMobilityPos (ParseResult pr)
{
  ...
  return pr.tokens.size () == 9 && pr.tokens[0] == NS2_NS_SCH && pr.tokens[1] == NS2_AT
         //    time             x coord          y coord            z coord          velocity are numbers?
         && pr.has_dval[2] && pr.has_dval[5] && pr.has_dval[6] && pr.has_dval[7] && pr.has_dval[8]
