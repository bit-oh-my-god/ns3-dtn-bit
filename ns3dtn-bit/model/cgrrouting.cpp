...
namespace ns3 {
    ...
    namespace ns3dtnbit {
        ...
        (node_id_t dest, dtn_time_t expiretime) {
            ...
            // remove old rrc and don't do it hot
            if (last_remove_table_time_ + NS3DTNBIT_CGR_OPTIMAL_OPTION_TABLE_REMOVE_INTERVAL < nowtime) {
                last_remove_table_time_ = nowtime;
                table_.erase(
                    std::remove_if(
                        table_.begin(), 
                        table_.end(), 
        ...
        void CGRRouting::CashedRouteTable::PushFinalXmit(CgrXmit cgrxmit,dtn_time_t local_forfeit_time, dtn_time_t local_best_delivery_time) {
            ...
            NS_LOG_INFO(LogPrefixMacro<<" cur_rrc.seq=" << cur_candidate_.seqpkt_ <<"after push final xmit,"
            << " rrc.path_ is " << cur_candidate_.dest_ << cur_candidate_.VecPathStr()
            << " rrc.vec_ is " << cur_candidate_.VecString());
    ...
    namespace ns3dtnbit {
        ...
        void CGRRouting::GetInfo(node_id_t destination_id, 
            ...
            // We take hopped node as exclude one to avoid loop-trans, it's like CGR-EncodeRoute
            for (auto const & node : exclude_node_of_thispkt) {
                if (node == from_id) {continue;}
                excluded_vec_.push_back(node);
            }
        ...
        void CGRRouting::ContactReviewProcedure(node_id_t cur_d, dtn_time_t cur_deadline, dtn_time_t best_deli) {
            ...
            if (cgr_find_proximate_count_ >= NS3DTNBIT_CGR_OPTIMAL_DECISION_AMOUNT) { return; }
            // 1.
...
#endif
            // CGR didn't mean anything unless 
        ...
        int CGRRouting::ForwardDecision(vector<RouteResultCandidate> & rrc_vec) {
            ...
            NS_LOG_DEBUG(LogPrefixMacro<<" forwarddecision: rrc_vec.size()=" << rrc_vec.size());
            if (rrc_vec.size() >= 2) {
                // multiplt choice
        ...
        int CGRRouting::NCMDecision(vector<RouteResultCandidate> const & rrc_vec) {
            ...
            //int ret = std::rand();
            // return less jump one is similar to CGR-ETO, to avoid loop problem, due to the order of CgrXmit, first one would have earlier forfeit_time
            int ret = 0;
            size_t last = 90000;    // big enough
            for (size_t i = 0; i < rrc_vec.size(); i++) {
                NS_LOG_DEBUG(LogPrefixMacro << "in CGRRouting::NCMDecision : no-"<<i <<" rrc ;this rrc.vecpathstr()=" << rrc_vec[i].VecPathStr());
                if (rrc_vec[i].GetXmitSize() < last) {
                    last = rrc_vec[i].GetXmitSize();
                    ret = i;
                }
            }
            NS_LOG_DEBUG(LogPrefixMacro<< "decision.rrc.vecpathstr()=" << rrc_vec[ret].VecPathStr());
